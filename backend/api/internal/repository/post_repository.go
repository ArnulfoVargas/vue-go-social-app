...
func (p *postRepository) GetPost(postId primitive.ObjectID) (model.Post, error) {
	ctx, cancel := helpers.GenerateContext()
	defer cancel()
	...
	if err != nil {
		...
		return model.Post{}, errors.New("error getting post")
	}
...
func (p *postRepository) UpdatePost(postId primitive.ObjectID, update bson.M) (model.Post, error) {
	ctx, cancel := helpers.GenerateContext()
	defer cancel()
	...
	if err != nil {
		return model.Post{}, fmt.Errorf("updatePost: %w", err)
	}

	return p.GetPost(postId)
}

func (p *postRepository) GetPostsByUserId(userId primitive.ObjectID) ([]model.Post, error) {
	ctx, cancel := helpers.GenerateContext()
	defer cancel()
...
func (p *postRepository) GetSuggestedPosts(userId primitive.ObjectID, limit int) ([]model.Post, error) {
	ctx, cancel := helpers.GenerateContext()
	defer cancel()
